...
const httpProductos = {
    ...
    postProductos: async (req, res) => {
        try {
            ...
            const productos = new Productos({ Codigo, Nombre, Descripcion, UnidadMedida, PrecioUnitario, Iva, Consumible });
            productos.save();
            res.json({ productos });
        } catch (error) {
            res.status(400).json({ error });
    ...
    putProductos: async (req, res) => {
        try {
            const { id } = req.params;
            const {Codigo, Nombre, Descripcion, UnidadMedida, PrecioUnitario, Iva, Consumible  } = req.body;
            const productos = await Productos.findByIdAndUpdate(id, {Codigo, Nombre, Descripcion, UnidadMedida, PrecioUnitario, Iva, Consumible  }, { new: true });
            res.json({ productos });
        } catch (error) {
